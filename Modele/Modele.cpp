    ...
    : currentLevel(std::make_unique<Level>("Tutorial", "Test level"))
{
    ...
    // - expresses ownership creation (make_unique) clearly and safely
    ...
    //
    // WHY unique_ptr + make_unique:
    // - Modele is the single owner of RoomManager
    // - no manual delete; safe on exceptions (RAII)
    roomManager = std::make_unique<RoomManager>(screenW, screenH);

    // MapManager handles floor/tile matrix and tile/wall textures.
    //
    // WHY unique_ptr:
    // - Modele owns MapManager lifetime; can reset/recreate when changing level
    ...
    // WHY prototypes stored as unique_ptr in a map:
    // - map key = enemy type string, easy lookup ("generic", "camera", "laser")
    // - unique_ptr expresses single ownership of each prototype instance
    ...
    for (const auto& p : tryPlayerPaths) {
        // WHY const auto&:
        // - avoids copying path strings in the loop
...
void Modele::reloadEnemiesForCurrentRoom()
{
    ...
    for (const auto& ed : roomManager->getCurrentRoomEnemies()) {
        // WHY pass enemyPrototypes by reference (in factory signature):
        // - avoid copying a map of unique_ptr (not copyable)
        // - allows lookup by enemy type and cloning the correct prototype
        auto e = createEnemyFromDefinition(ed, enemyPrototypes, scaleW, scaleH);

        // WHY std::move(e):
        // - unique_ptr is move-only; move transfers ownership into the enemies vector
...
void Modele::updateEnemies()
{
    ...
    for (auto& e : enemies) {
        // WHY pass joueur by reference in detectPlayer signature (Enemy side):
        // - enemy needs to read current player hitbox state without copying the shape
        e->update();
        e->updateAnimation();
...
const std::vector<std::unique_ptr<sf::Shape>>& Modele::getObstacleShapes() const
{
    ...
    // - vector contains unique_ptr (move-only), cannot be copied
    // - avoids transferring ownership; caller only reads shapes
...
{
    // Returned by value: caller gets an independent string snapshot
...
std::vector<Objective>& Modele::getCurrentRoomObjectives()
{
    ...
    // - gameplay needs to modify Objective objects stored in the room
    // - avoids copies of Objective (which includes SFML resources and custom move/copy logic)
...
std::vector<Objective> Modele::getAllLevelObjectives() const
{
    ...
    // WHY return by value:
    // - caller gets a snapshot independent of internal room storage
...
{
    // Returned as const reference to avoid copying move-only unique_ptr elements
    return enemies;
}
...
bool Modele::changeRoom(int newRoomIndex, const std::string& entryDirection)
{
    ...
    //
    // WHY pass entryDirection as const ref:
    // - avoid copying string
    // WHY pass joueur by reference (RoomManager side):
    // - RoomManager needs to reposition the actual player hitbox shape in-place
...
void Modele::markCurrentRoomDialogueShown()
{
    ...
    // WHY store in set<int>:
    // - prevents duplicates automatically
    // - fast membership check for "already shown"
...
{
    // WHY store pointer:
    // - non-owning reference to an Objective stored in a room vector
    // - allows modifying the original object (setAccomplished, dialogue flags) without copying
...
{
    // WHY return by value:
    // - provides an independent snapshot; caller cannot mutate internal vector directly
    return playerScores;
}
...
bool Modele::loadLevelFromFile(const std::string& levelJsonPath)
{
    ...
    //
    // WHY reset + recreate:
    // - guarantees no stale room data (doors/objectives/enemy definitions) remain
    // - fresh state is important when loading a new level file
    roomManager.reset();
    roomManager = std::make_unique<RoomManager>(screenW, screenH);
...
void Modele::setFloorMatrix(const std::vector<std::vector<int>>& m)
{
    ...
    // - avoids copying a potentially large matrix
...
void Modele::reset()
{
    ...
    // WHY roomManager.reset() then make_unique:
    // - hard reset of room/objective/enemy-definition state
    // - ensures no stale vectors/pointers remain after a restart
    roomManager.reset();
    roomManager = std::make_unique<RoomManager>(screenW, screenH);
