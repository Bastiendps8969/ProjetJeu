...
namespace Modele
{
    ...
    void GenericEnemy::update()
    {
        ...
        // WHY update() sets isMoving + row:
        // - isMoving drives updateAnimation() (walk vs idle)
        // - row selects which sprite-sheet row to display (directional animation)
    }
    ...
    std::unique_ptr<Enemy> GenericEnemy::clone() const
    {
        ...
        //
        // WHY std::make_unique<GenericEnemy>(*this):
        // - allocates a new object on the heap and copies the current state
        // - returns ownership via unique_ptr (RAII)
        // - avoids slicing: the dynamic type remains GenericEnemy
    ...
    {
        // WHY joueur is passed as const reference:
        // - detection is read-only
        // - avoids copying the RectangleShape (performance)
        ...
        // - relies on direction being normalized for correct cosine comparison
    }
    ...
    void CameraEnemy::detectPlayer(const sf::RectangleShape& joueur)
    {
        ...
        // - keeps behavior consistent across "moving enemy" and "stationary camera"
    }
    ...
    std::unique_ptr<Enemy> CameraEnemy::clone() const
    {
        // Prototype clone
        //
        // WHY make_unique<CameraEnemy>(*this):
        // - preserves derived type
    ...
    void LaserEnemy::detectPlayer(const sf::RectangleShape& joueur)
    {
        ...
        // - proj checks distance along beam axis; side checks thickness around the axis
    }
...
namespace Modele
{
    ...
    {
        // WHY ed passed as const reference:
        // - EnemyDefinition can contain vectors (patrolPoints); avoid copy
        // - factory reads configuration only (no mutation)
        //
        // WHY prototypes passed as const reference:
        // - map contains unique_ptr (move-only)
        // - factory must not mutate or consume prototypes, only clone from them
        ...
        if (it != prototypes.end())
        {
            ...
            //
            // WHY clone() instead of copying base:
            // - avoids slicing
            // - returns correct derived type wrapped in unique_ptr
            e = it->second->clone();
        }
        else
        {
        ...
        if (type == "camera")
        {
            ...
            if (camLoaded)
            {
                ...
                if (cc)
                {
                    ...
                    // - factory has only Enemy* at compile-time
                    // - needs access to derived-only data (facing)
                }
            }
        }
        else
        {
            ...
            // WHY setOrigin center:
            // - simplifies transforms (rotation around center, positioning by center)
        }
        ...
        if (type == "generic")
        {
            ...
            if (g)
            {
                ...
                // WHY set initial direction:
                // - ensures cone/beam math and animation row have a meaningful forward vector immediately
            }
        ...
        else if (type == "camera")
        {
            ...
            if (c)
            {
                ...
                // WHY store direction as vector:
                // - detection math uses dot products/projections which are simpler with vectors than strings
            }
        ...
        else if (type == "laser")
        {
            ...
            if (l)
            {
                ...
                // - beam detection uses projection on direction (fast and robust)
            }
        ...
        //
        // WHY return unique_ptr by value:
        // - transfers ownership safely (move)
        // - caller becomes responsible for the runtime enemy lifetime via RAII
        return e;
    }
