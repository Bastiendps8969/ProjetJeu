...
bool RoomManager::loadRoomsFromJson(const std::string& filename)
{
    ...
    try
    {
        ...
        for (auto it = j.begin(); it != j.end(); ++it)
        {
            ...
            {
                // Apply scaling so obstacle coordinates follow the current screen size
                const float refW = 2560.f; // same reference as used for enemies
            ...
            if (roomJson.contains("objectives")) {
                // Use same reference scaling as for obstacles/enemies so coordinates match
